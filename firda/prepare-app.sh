...
DEFAULT_SOURCE="$REPO_DIR/TickTick_8.0.60_468/TickTick/TickTick.app"
SOURCE_INPUT="${1:-$DEFAULT_SOURCE}"
APP="${2:-$REPO_DIR/TickTick.debug.app}"
...
Defaults:
  ...
  OUTPUT_APP $REPO_DIR/TickTick.debug.app

Examples:
  $0
  $0 TickTick_8.0.60_468.dmg
  $0 /Applications/TickTick.app /tmp/TickTick.debug.app
USAGE
}
...
echo "[1/4] Creating clean debug copy"
if [[ -e "$APP" ]]; then
  rm -rf "$APP"
fi
